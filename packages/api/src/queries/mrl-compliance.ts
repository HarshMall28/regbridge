...
const APPLICABILITY_MAP: Record<number, "current" | "future" | "previous"> = {
  0: "future",
  1: "current",
  2: "previous",
};
...
> {
  ...
  if (/^\d+$/.test(trimmed)) {
    const row = await db
      ...
      .select(["commodity_id", "product_name", "product_code", "parent_id"])
...
): Promise<{ mrl: MrlResult | null; inherited: boolean; parentName: string | null }> {
  // Try direct match
  ...
  if (directRow) {
    return {
      mrl: {
        ...
        applicability: APPLICABILITY_MAP[directRow.applicability ?? 1] ?? "current",
    ...
    if (parentRow) {
      ...
      return {
        mrl: {
          ...
          applicability: APPLICABILITY_MAP[parentRow.applicability ?? 1] ?? "current",
...
export async function checkMrlCompliance(
  ...
  if (!substanceRow) {
    throw new Error(`Substance as_id=${resolved.as_id} not found in database`);
  }
  ...
  const mrlType: "default" | "specific" | "unresolved" =
    prl?.startsWith("Default") ? "default" : residueId != null ? "specific" : "unresolved";
  ...
  if (mrlType === "default") {
    const response: MrlCheckResponse = {
      ...
      default_mrl: {
        value: 0.01,
        unit: "mg/kg",
        regulation_text: prl ?? "Default MRL of 0.01 mg/kg according to Art 18(1)(b) Reg 396 / 2005",
      },
    };
  ...
  const residueRow = await db
    ...
    .select(["residue_id", "residue_name", "footnote_code", "footnote_def"])
    .where("residue_id", "=", residueId!)
    .executeTakeFirst();
    ...
    return {
      ...
      residue_definition: residueRow
        ? {
            ...
            footnote: [residueRow.footnote_code, residueRow.footnote_def]
              .filter(Boolean)
              .join(" — ") || null,
          }
        : null,
